...
#进行用户信息删除的操作
def deleteusetinfoop(request):
    #获取传送过来的uid参数
    useruid = request.GET.get('op');
    #进行对应信息的用户删除
    currentuser = User.objects.filter(uid = useruid)[0]; #得到当前的这个用户
    currentuser.delete();  #删除用户信息
    result = 'success';
    data = {'result':result};  #封装结果
    return HttpResponse(JsonResponse(data), content_type="application/json")

#进行用户信息更新的操作
def updatauserinfoop(request):
    #拿取传送的参数
    userid = request.GET.get('uid');  #因为uid不能进行更新，所以通过这个主键来进行用户信息的更新
    username = request.GET.get('name');
    usercla = request.GET.get('cla');
    usertel = request.GET.get('tel');
    userper = request.GET.get('per');
    currentUser = User.objects.filter( uid = userid)[0];
    currentUser.name = username;
    if userper == "1" or userper == "2":  #学生或者助教
        currentUser.student_class_name = usercla;
    elif userper == "3":  #老师的所带的班级
        #进行班级的分割,通过-符号，将字符串转为列表
        currentUser.teacher_class_name = usercla.split('-');
    currentUser.contact = usertel;
    currentUser.permission = userper;
    currentUser.save();
    data = {'result':"success"};
    return HttpResponse(JsonResponse(data), content_type="application/json");